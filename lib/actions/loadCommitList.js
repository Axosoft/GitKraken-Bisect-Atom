...
export const loadCommitList = async (repo) => {
  if (store.getState().state !== StateConstants.INIT) {
    return;
  }

  store.move(StateConstants.COMMIT_LIST_LOADING, { repo });
  ...
  const commitList = [...fixTopologyWalk(head)];
