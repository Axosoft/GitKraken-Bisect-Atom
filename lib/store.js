...
const getUnknownShasInRange = (head, isInRangeBySha) => {
  const unknownShasInRange = {};

  let nextNode = head;

  while (nextNode && isInRangeBySha[nextNode.commit.sha]) {
    unknownShasInRange[nextNode.commit.sha] = BisectConstants.commitState.UNKNOWN;

    if (nextNode.parents.length === 0) {
      break;
    }
    if (nextNode.parents.length === 1) {
      nextNode = nextNode.parents[0];
    } else {
      return R.reduce(
        (unknownShasInRange, parent) => R.merge(unknownShasInRange, getUnknownShasInRange(parent, isInRangeBySha)),
        unknownShasInRange,
        nextNode.parents
      );
    }
...
const stateTransitions = {
  ...
  [StateConstants.COMMIT_2_SELECTED]: {
    ...
    [StateConstants.BISECT_CHECKOUT_TRIGGERED]: (prevState) => {
      ...
      const unknownShasInRange = getUnknownShasInRange(R.prop(end, commitNodeBySha), isInSelectionRangeBySha);
