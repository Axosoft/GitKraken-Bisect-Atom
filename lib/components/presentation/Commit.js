...
const getCommitRowClassName = (isSelected, isCurrent, status) => classnames(
  'commit-row',
  {
    'commit-row-selected': isSelected,
    'commit-row-current': isCurrent,
...
export default class Commit extends PureComponent {
  render() {
    const { commit, isCurrent, isSelected, onClick, status, style } = this.props;

    return (
      <div onClick={() => onClick(commit.sha)} style={style}>
        <div className={getCommitRowClassName(isSelected, isCurrent, status)} style={COMMIT_ROW_STYLE}>
